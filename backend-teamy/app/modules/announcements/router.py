...
from app.core.security import create_announcement_socket_ticket, decode_announcement_socket_ticket, decode_session_token
...
from app.modules.notifications.services import send_announcement_email_to_project_members
...
async def serialize_announcement(
    db: AsyncSession, announcement: Announcement, user_id: UUID, is_read: bool | None = None
) -> AnnouncementResponse:
    creator = await db.get(User, announcement.created_by_user_id)
    if creator is None:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Announcement creator could not be loaded"
        )
    member_result = await db.execute(
        select(ProjectMember).where(
            ProjectMember.project_id == announcement.project_id, ProjectMember.user_id == creator.id
        )
    )
...
async def get_announcement_for_project(db: AsyncSession, project_id: UUID, announcement_id: UUID) -> Announcement:
    result = await db.execute(
        select(Announcement).where(Announcement.project_id == project_id, Announcement.id == announcement_id)
    )
...
def require_announcement_manager(project_member: ProjectMember, announcement: Announcement, user: User) -> None:
    if project_member.role not in ("leader", "co_leader") and announcement.created_by_user_id != user.id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only the announcement creator or project leader can edit this announcement",
        )


async def serialize_announcements_batch(
    db: AsyncSession, announcements: list[Announcement], user_id: UUID
...
async def list_announcements(
    ...
    cache_key = f"project:{project.id}:announcements:{user.id}:l={limit}:o={offset}"
    cached = await cache_get(cache_key)
    if cached is not None:
        return AnnouncementListResponse(**cached)
    ...
    if did_sync_state:
        await db.commit()
    response = AnnouncementListResponse(announcements=await serialize_announcements_batch(db, announcements, user.id))
...
async def create_announcement(
    ...
    if payload.is_record_only and project_member.role not in ("leader", "co_leader"):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only project leaders or co-leaders can create record-only announcements",
        )
    ...
    if payload.is_record_only:
        ...
        await create_user_notifications(
            ...
            body=announcement.body,
            target_path=f"/projects/{project.slug}/announcements",
    ...
    background_tasks.add_task(
        manager.broadcast, project.id, {"event": "announcement.created", "announcement": broadcast_announcement}
    )
    return response
...
async def update_announcement(
    ...
    if payload.is_pinned is not None:
        if announcement.is_record_only and payload.is_pinned:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Record-only announcements cannot be pinned"
            )
    ...
    if payload.is_record_only is not None and payload.is_record_only != announcement.is_record_only:
        if project_member.role not in ("leader", "co_leader"):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Only project leaders or co-leaders can update record-only state",
            )
        announcement.is_record_only = payload.is_record_only
        if announcement.is_record_only:
            announcement.is_pinned = False
            await mark_announcement_read_for_users(
                db, announcement.id, await get_project_member_user_ids(db, project.id)
            )
    sync_automatic_announcement_state(announcement)
    ...
    broadcast_announcement = response.model_copy(update={"is_read": True if announcement.is_record_only else False})
    background_tasks.add_task(
        manager.broadcast, project.id, {"event": "announcement.updated", "announcement": broadcast_announcement}
    )
    return response
...
async def mark_announcement_deadline_done(
    ...
    if announcement.deadline_date is not None:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Only announcements without a date can be marked done"
        )
...
async def delete_announcement(
    ...
    await invalidate_project(project.id)
    background_tasks.add_task(
        manager.broadcast, project.id, {"event": "announcement.deleted", "announcement_id": announcement_id}
    )
