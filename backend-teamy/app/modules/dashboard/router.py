...
from app.modules.announcements.router import serialize_announcements_batch
...
from app.modules.projects.dependencies import get_project_membership
from app.modules.projects.models import Project, ProjectMember
from app.modules.tasks.models import Task, TaskAssignee
from app.modules.tasks.router import serialize_tasks_batch
...
async def get_user_member_maps(
    db: AsyncSession, project_id: UUID, user_ids: set[UUID]
) -> tuple[dict[UUID, User], dict[UUID, ProjectMember]]:
    if not user_ids:
        return {}, {}
    ...
    member_result = await db.execute(
        select(ProjectMember).where(ProjectMember.project_id == project_id, ProjectMember.user_id.in_(user_ids))
    )
...
async def build_activity_items(
    ...
    for announcement in announcements:
        actor = display_name(
            users_by_id.get(announcement.created_by_user_id), members_by_user_id.get(announcement.created_by_user_id)
        )
        target_path = f"/projects/{project_slug}/announcements"
        ...
        if timestamps_differ(announcement.created_at, announcement.updated_at):
            activity_items.append(
                DashboardActivityItemResponse(
                    ...
                    kind="announcement",
                    title=announcement.title,
                    description="pinned or updated a project announcement"
                    if announcement.is_pinned
                    else "updated a project announcement",
                    actor=actor,
    ...
    for task in tasks:
        creator = display_name(
            users_by_id.get(task.created_by_user_id), members_by_user_id.get(task.created_by_user_id)
        )
        ...
        target_path = f"/projects/{project_slug}/task-board"
        activity_items.append(
            DashboardActivityItemResponse(
    ...
    for file in files:
        ...
        target_path = (
            f"/projects/{project_slug}/file-hub/{file.id}"
            if file.kind == "doc"
            else f"/projects/{project_slug}/file-hub"
        )
        activity_items.append(
            DashboardActivityItemResponse(
...
async def get_dashboard_summary(
    ...
    cache_key = f"project:{project.id}:dashboard:{user.id}"
    ...
    active_task_count_result = await db.execute(
        select(func.count())
        .select_from(Task)
        .where(Task.project_id == project.id, Task.is_private.is_(False), Task.status != "done")
    )
    ...
    task_deadline_result = await db.execute(
        select(Task)
        .where(
            Task.project_id == project.id, Task.is_private.is_(False), Task.status != "done", Task.due_date.is_not(None)
        )
    ...
    deadline_items = [
        DashboardDeadlineItemResponse(
            id=f"task:{task.id}", source_id=task.id, kind="task", title=task.title, due_date=task.due_date
        )
    ...
    deadline_items = sorted(
        deadline_items, key=lambda item: (item.due_date is not None, item.due_date or date.min, item.title)
    )[:4]

    activity_announcement_result = await db.execute(
        select(Announcement)
        .where(Announcement.project_id == project.id)
        .order_by(Announcement.updated_at.desc(), Announcement.created_at.desc())
        .limit(10)
    )
    ...
    activity_file_result = await db.execute(
        select(FileResource)
        .where(FileResource.project_id == project.id)
