...
from app.modules.filehub.schemas import (
...
from app.modules.tasks.models import Task, TaskFileLink
...
async def serialize_file_resource(
    db: AsyncSession, resource: FileResource, include_content: bool = False
) -> FileResourceSummaryResponse:
    creator = await db.get(User, resource.created_by_user_id)
    if creator is None:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="File creator could not be loaded"
        )
    member_result = await db.execute(
        select(ProjectMember).where(
            ProjectMember.project_id == resource.project_id, ProjectMember.user_id == creator.id
        )
    )
    ...
    return FileResourceSummaryResponse(
        id=resource.id,
        project_id=resource.project_id,
        title=resource.title,
        kind=resource.kind,
...
async def get_file_for_project(db: AsyncSession, project_id: UUID, file_id: UUID) -> FileResource:
    result = await db.execute(
        select(FileResource).where(FileResource.project_id == project_id, FileResource.id == file_id)
    )
...
async def list_file_resources(
    ...
    cache_key = f"project:{project.id}:files:l={limit}:o={offset}"
    cached = await cache_get(cache_key)
    if cached is not None:
        return FileResourceListResponse(**cached)
    ...
    response = FileResourceListResponse(files=await serialize_file_resources_batch(db, list(resources)))
