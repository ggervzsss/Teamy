...
from app.core.cloudinary import delete_profile_avatar, upload_profile_avatar
...
from app.core.user_responses import serialize_account_user
from app.modules.auth.models import User
from app.modules.auth.schemas import AuthResponse, UserProfileUpdateRequest
...
async def restore_my_google_avatar(
    ...
    if not user.google_avatar_url:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="No Google profile picture is available for this account"
        )
...
async def google_login(settings: Settings = Depends(get_settings)) -> RedirectResponse:
    if not settings.google_client_id or not settings.google_client_secret:
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="Google authentication is not configured"
        )
...
async def google_callback(
    ...
    callback_url = f"{str(settings.frontend_url).rstrip('/')}/auth/callback#token={token}"
