...
from app.core.cache import LONG_TTL_SECONDS, cache_get, cache_set, invalidate_project, invalidate_user
from app.core.cloudinary import delete_project_avatar, delete_task_image_from_cloudinary, upload_project_avatar
...
from app.modules.team.router import broadcast_member_joined
...
async def generate_teamy_code(db: AsyncSession) -> str:
    for _ in range(10):
        ...
        if result.scalar_one_or_none() is None:
            return code
    raise HTTPException(
        status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Could not generate a unique Teamy code"
    )
...
async def invalidate_project_members_cache(db: AsyncSession, project_id: UUID) -> None:
...
def parse_uuid(value: object, field_name: str) -> UUID:
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail=f"Invalid backup UUID for {field_name}"
        ) from exc
...
def parse_backup_datetime(value: object, field_name: str, required: bool = True) -> datetime | None:
    if value is None:
        if required:
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail=f"Missing backup datetime for {field_name}"
            )
        return None
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail=f"Invalid backup datetime for {field_name}"
        ) from exc
...
def parse_backup_date(value: object, field_name: str, required: bool = True) -> date | None:
    if value is None:
        if required:
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail=f"Missing backup date for {field_name}"
            )
        return None
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail=f"Invalid backup date for {field_name}"
        ) from exc
...
def backup_records(payload: dict[str, object], key: str) -> list[dict[str, object]]:
    ...
    if not isinstance(value, list) or any(not isinstance(item, dict) for item in value):
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail=f"Backup field {key} must be a list"
        )
    return value
...
async def list_projects(
    ...
    await cache_set(cache_key, jsonable_encoder(response), ttl=LONG_TTL_SECONDS)
    return response
...
async def create_project(
    ...
    project = Project(
        ...
        slug=generate_slug(payload.name, teamy_code),
...
async def import_project_backup(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="Upload a valid Teamy project backup JSON file"
        ) from exc
    ...
    if existing_project is not None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT, detail="This project backup has already been imported"
        )

    teamy_code = str(project_record.get("teamy_code") or "").strip()
    if not teamy_code:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="Backup project is missing its Teamy code"
        )
    ...
    if code_result.scalar_one_or_none() is not None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT, detail="A project with this Teamy code already exists"
        )
    ...
    for user_record in backup_users:
        ...
        if not email:
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="Backup user is missing an email"
            )
        ...
        users_to_create.append(
            User(
                ...
                google_avatar_url=str(user_record["google_avatar_url"])
                if user_record.get("google_avatar_url")
                else None,
                last_online_at=parse_backup_datetime(
                    user_record.get("last_online_at"), "users.last_online_at", required=False
                ),
                created_at=parse_backup_datetime(user_record.get("created_at"), "users.created_at")
                or datetime.now(UTC),
                updated_at=parse_backup_datetime(user_record.get("updated_at"), "users.updated_at")
                or datetime.now(UTC),
            )
        )
    ...
    def mapped_user_id(value: object, field_name: str, fallback: UUID | None = None) -> UUID:
        ...
        if mapped_id is None:
            ...
            raise HTTPException(
                ...
                detail=f"Backup references unknown user in {field_name}",
            )
        return mapped_id

    project_name = str(project_record.get("name") or "Imported Workspace")[:160]
    project = Project(
        id=project_id,
        name=project_name,
        description=str(project_record["description"]) if project_record.get("description") is not None else None,
        slug=generate_slug(project_name, teamy_code),
        teamy_code=teamy_code,
        created_by_user_id=mapped_user_id(
            project_record.get("created_by_user_id"), "project.created_by_user_id", fallback=user.id
        ),
    ...
    if current_user_membership is None:
        current_user_membership = ProjectMember(
            project_id=project.id, user_id=user.id, role="leader", joined_at=datetime.now(UTC)
        )
    ...
    for task_record in backup_records(raw_backup, "tasks"):
        ...
        db.add(
            Task(
                ...
                created_by_user_id=mapped_user_id(
                    task_record.get("created_by_user_id"), "tasks.created_by_user_id", fallback=user.id
                ),
                reviewed_by_user_id=mapped_user_id(task_record["reviewed_by_user_id"], "tasks.reviewed_by_user_id")
                if task_record.get("reviewed_by_user_id")
                else None,
                reviewed_at=parse_backup_datetime(task_record.get("reviewed_at"), "tasks.reviewed_at", required=False),
                review_remarks=str(task_record["review_remarks"])
                if task_record.get("review_remarks") is not None
                else None,
                created_at=parse_backup_datetime(task_record.get("created_at"), "tasks.created_at")
                or datetime.now(UTC),
                updated_at=parse_backup_datetime(task_record.get("updated_at"), "tasks.updated_at")
                or datetime.now(UTC),
            )
        )
    ...
    for file_record in backup_records(raw_backup, "file_resources"):
        ...
        db.add(
            FileResource(
                ...
                created_by_user_id=mapped_user_id(
                    file_record.get("created_by_user_id"), "file_resources.created_by_user_id", fallback=user.id
                ),
                created_at=parse_backup_datetime(file_record.get("created_at"), "file_resources.created_at")
                or datetime.now(UTC),
                updated_at=parse_backup_datetime(file_record.get("updated_at"), "file_resources.updated_at")
                or datetime.now(UTC),
            )
        )
    ...
    for announcement_record in backup_records(raw_backup, "announcements"):
        ...
        db.add(
            Announcement(
                ...
                deadline_date=parse_backup_date(
                    announcement_record.get("deadline_date"), "announcements.deadline_date", required=False
                ),
                deadline_done_at=parse_backup_datetime(
                    announcement_record.get("deadline_done_at"), "announcements.deadline_done_at", required=False
                ),
                is_record_only=bool(announcement_record.get("is_record_only", False)),
                created_by_user_id=mapped_user_id(
                    announcement_record.get("created_by_user_id"), "announcements.created_by_user_id", fallback=user.id
                ),
                created_at=parse_backup_datetime(announcement_record.get("created_at"), "announcements.created_at")
                or datetime.now(UTC),
                updated_at=parse_backup_datetime(announcement_record.get("updated_at"), "announcements.updated_at")
                or datetime.now(UTC),
            )
        )
    ...
    for assignee_record in backup_records(raw_backup, "task_assignees"):
        ...
        db.add(
            TaskAssignee(
                ...
                completed_at=parse_backup_datetime(
                    assignee_record.get("completed_at"), "task_assignees.completed_at", required=False
                ),
                created_at=parse_backup_datetime(assignee_record.get("created_at"), "task_assignees.created_at")
                or datetime.now(UTC),
                updated_at=parse_backup_datetime(assignee_record.get("updated_at"), "task_assignees.updated_at")
                or datetime.now(UTC),
            )
        )
    ...
    for link_record in backup_records(raw_backup, "task_file_links"):
        ...
        db.add(
            TaskFileLink(
                ...
                created_at=parse_backup_datetime(link_record.get("created_at"), "task_file_links.created_at")
                or datetime.now(UTC),
            )
        )
    ...
    for read_record in backup_records(raw_backup, "announcement_reads"):
        ...
        db.add(
            AnnouncementRead(
                ...
                read_at=parse_backup_datetime(read_record.get("read_at"), "announcement_reads.read_at")
                or datetime.now(UTC),
            )
        )
    ...
    for notification_record in backup_records(raw_backup, "notifications"):
        db.add(
            Notification(
                ...
                target_path=str(notification_record["target_path"])[:1024]
                if notification_record.get("target_path") is not None
                else None,
                is_email_backed=bool(notification_record.get("is_email_backed", False)),
                read_at=parse_backup_datetime(
                    notification_record.get("read_at"), "notifications.read_at", required=False
                ),
                created_at=parse_backup_datetime(notification_record.get("created_at"), "notifications.created_at")
                or datetime.now(UTC),
            )
        )
...
async def get_project_by_slug(
    ...
    cache_key = f"project:{project.id}:detail:{user.id}"
    cached = await cache_get(cache_key)
    if cached is not None:
        return ProjectResponse(**cached)
    response = await serialize_project(db, project, membership)
    await cache_set(cache_key, jsonable_encoder(response), ttl=LONG_TTL_SECONDS)
    return response
...
async def get_project(
    ...
    if cached is not None:
        return ProjectResponse(**cached)
    response = await serialize_project(db, project, project_member)
    await cache_set(cache_key, jsonable_encoder(response), ttl=LONG_TTL_SECONDS)
    return response
...
async def update_project(
    ...
    if payload.name is not None:
        ...
        project.slug = generate_slug(next_name, project.teamy_code)

    if payload.description is not None:
        project.description = payload.description.strip() if payload.description.strip() else None

    if payload.icon_url is not None:
        new_icon_url = payload.icon_url.strip() if payload.icon_url.strip() else None
        if project.icon_public_id and (
            not new_icon_url or not (new_icon_url.startswith("http://") or new_icon_url.startswith("https://"))
        ):
            try:
                await delete_project_avatar(settings, project.icon_public_id)
            except HTTPException:
                pass
            project.icon_public_id = None
        project.icon_url = new_icon_url

    if payload.color_theme is not None:
        project.color_theme = payload.color_theme.strip() if payload.color_theme.strip() else None

    await db.commit()
    await invalidate_project_members_cache(db, project.id)
    await db.refresh(project)
...
async def upload_project_icon(
    ...
    icon_url, public_id = await upload_project_avatar(settings, project.id, file)
    if project.icon_public_id and project.icon_public_id != public_id:
        try:
            await delete_project_avatar(settings, project.icon_public_id)
        except HTTPException:
            pass
    ...
    project.icon_public_id = public_id
...
async def export_project_backup(
    ...
    member_result = await db.execute(
        select(ProjectMember).where(ProjectMember.project_id == project.id).order_by(ProjectMember.joined_at.asc())
    )
    ...
    if task_ids:
        assignee_result = await db.execute(
            select(TaskAssignee).where(TaskAssignee.task_id.in_(task_ids)).order_by(TaskAssignee.created_at.asc())
        )
        task_assignees = list(assignee_result.scalars().all())
        link_result = await db.execute(
            select(TaskFileLink).where(TaskFileLink.task_id.in_(task_ids)).order_by(TaskFileLink.created_at.asc())
        )
    ...
    file_result = await db.execute(
        select(FileResource).where(FileResource.project_id == project.id).order_by(FileResource.created_at.asc())
    )
    files = list(file_result.scalars().all())

    announcement_result = await db.execute(
        select(Announcement).where(Announcement.project_id == project.id).order_by(Announcement.created_at.asc())
    )
    ...
    if announcement_ids:
        read_result = await db.execute(
            ...
            .order_by(AnnouncementRead.read_at.asc())
        )
    ...
    notification_result = await db.execute(
        select(Notification).where(Notification.project_id == project.id).order_by(Notification.created_at.asc())
    )
    ...
    backup = {
        ...
        "members": [
            export_record(member, ("id", "project_id", "user_id", "role", "nickname", "joined_at"))
        ...
        "task_assignees": [
            export_record(assignee, ("id", "task_id", "user_id", "status", "completed_at", "created_at", "updated_at"))
        ...
        "task_file_links": [
            export_record(link, ("id", "task_id", "file_resource_id", "created_at")) for link in task_file_links
        ],
        "announcements": [
            export_record(
                announcement,
        ...
        "announcement_reads": [
            export_record(read, ("id", "announcement_id", "user_id", "read_at")) for read in announcement_reads
        ],
        "notifications": [
            export_record(
                notification,
    ...
    safe_project_name = (
        "-".join(
            filter(
                None,
                ("".join(character.lower() if character.isalnum() else "-" for character in project.name)).split("-"),
            )
        )
        or "workspace"
    )
...
async def delete_project(
    ...
    task_images_result = await db.execute(
        select(TaskImage.cloudinary_public_id)
        .join(Task, TaskImage.task_id == Task.id)
        .where(Task.project_id == project.id, TaskImage.cloudinary_public_id.isnot(None))
    )
