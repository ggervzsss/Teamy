...
def decode_announcement_socket_ticket(ticket: str, settings: Settings) -> tuple[UUID, UUID]:
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid announcement socket ticket"
        ) from exc
...
def decode_notification_socket_ticket(ticket: str, settings: Settings) -> UUID:
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid notification socket ticket"
        ) from exc
