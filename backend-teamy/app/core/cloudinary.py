...
def sign_cloudinary_params(params: dict[str, str], api_secret: str) -> str:
    ...
    return sha1(f"{serialized}{api_secret}".encode()).hexdigest()
...
async def read_avatar_file(file: UploadFile) -> bytes:
    ...
    if len(content) > MAX_AVATAR_BYTES:
        raise HTTPException(
            status_code=status.HTTP_413_REQUEST_ENTITY_TOO_LARGE, detail="Profile photo must be 5MB or smaller"
        )
    return content
...
async def upload_project_avatar(settings: Settings, project_id: UUID, file: UploadFile) -> tuple[str, str]:
    ensure_cloudinary_configured(settings)
    content = await read_avatar_file(file)
    timestamp = str(int(time()))
    public_id = f"teamy/projects/{project_id}"
    params = {
        "invalidate": "true",
        "overwrite": "true",
        "public_id": public_id,
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        response = await client.post(
            ...
            files={"file": (file.filename or "project_icon", content, file.content_type)},
        )
...
async def delete_project_avatar(settings: Settings, public_id: str) -> None:
    ensure_cloudinary_configured(settings)
    timestamp = str(int(time()))
    params = {
        "invalidate": "true",
...
async def read_task_image_file(file: UploadFile) -> bytes:
    ...
    if len(content) > MAX_TASK_IMAGE_BYTES:
        raise HTTPException(
            status_code=status.HTTP_413_REQUEST_ENTITY_TOO_LARGE, detail="Image must be 10 MB or smaller"
        )
    return content


async def upload_task_image(settings: Settings, task_id: UUID, file: UploadFile) -> tuple[str, str]:
    """Upload an image for a ticket and return (secure_url, public_id)."""
    ensure_cloudinary_configured(settings)
    content = await read_task_image_file(file)
    timestamp = str(int(time()))
    image_uuid = str(uuid4())
    public_id = f"teamy/task-images/{task_id}/{image_uuid}"
    params = {
        "public_id": public_id,
        "timestamp": timestamp,
    }
    ...
    async with httpx.AsyncClient(timeout=60) as client:
        response = await client.post(
            ...
            files={"file": (file.filename or "image", content, file.content_type)},
        )
...
async def delete_task_image_from_cloudinary(settings: Settings, public_id: str) -> None:
    """Delete a task image from Cloudinary (best-effort, errors are suppressed)."""
    if not settings.cloudinary_cloud_name or not settings.cloudinary_api_key or not settings.cloudinary_api_secret:
        return
    timestamp = str(int(time()))
    params = {
