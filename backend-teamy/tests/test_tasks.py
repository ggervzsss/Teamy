...
async def test_member_cannot_create_done_task_directly(client):
    ...
    assert response.json()["detail"] == "Only project leaders or co-leaders can perform this action"
...
async def test_private_tasks_are_only_visible_in_my_tasks(client):
    ...
    completed = await client.patch(
        f"/projects/{project['id']}/tasks/{private_task['id']}/assignees/me", json={"status": "done"}
    )
    ...
    member_two_update = await client.patch(
        f"/projects/{project['id']}/tasks/{private_task['id']}/assignees/me", json={"status": "in_progress"}
    )
    ...
    leader_update = await client.patch(
        f"/projects/{project['id']}/tasks/{private_task['id']}", json={"title": "Leader should not see this"}
    )
...
async def test_member_readiness_requires_explicit_submit_for_review(client):
    ...
    first_update = await client.patch(
        f"/projects/{project['id']}/tasks/{task_id}/assignees/me", json={"status": "ready_for_review"}
    )
...
async def test_request_changes_resets_assignees_to_progress(client):
    ...
    reviewed = await client.post(
        f"/projects/{project['id']}/tasks/{task_id}/review",
        json={"action": "request_changes", "remarks": "Please tighten the conclusion."},
    )
...
async def test_task_stays_in_progress_until_every_assignee_is_ready(client):
    ...
    created = await client.post(
        f"/projects/{project['id']}/tasks",
        json={
            "title": "Leader final pass",
            "assignee_ids": [leader["id"], member_one["id"]],
            "initial_status": "in_progress",
        },
    )
    task_id = created.json()["id"]
...
async def test_creator_or_leader_can_edit_task_details(client):
    ...
    assert {assignee["user"]["id"] for assignee in creator_update.json()["assignees"]} == {
        member_one["id"],
        member_two["id"],
    }
    await logout(client)
    ...
    await login(client, leader["email"], leader["full_name"])
    leader_update = await client.patch(
        f"/projects/{project['id']}/tasks/{task_id}", json={"description": "Use the latest project template."}
    )
