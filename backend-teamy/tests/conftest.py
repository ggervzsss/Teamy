...
class AsyncUserHelper(Protocol):
    def __call__(self, email: str, full_name: str = "Test User") -> Awaitable[User]: ...


class TeamyTestClient(AsyncClient):
    _teamy_settings: Settings
    _sessionmaker: async_sessionmaker[AsyncSession]
    _get_or_create_user: AsyncUserHelper
    _login_user: AsyncUserHelper
    _logout_user: Callable[[], None]


@pytest_asyncio.fixture
async def client(monkeypatch) -> AsyncGenerator[TeamyTestClient, None]:
    engine = create_async_engine("sqlite+aiosqlite:///:memory:", connect_args={"check_same_thread": False})
    sessionmaker = async_sessionmaker(engine, expire_on_commit=False)
    monkeypatch.setattr("app.core.database.SessionLocal", sessionmaker)
    settings = Settings(
    ...
    async with TeamyTestClient(transport=ASGITransport(app=app), base_url="http://testserver") as test_client:
