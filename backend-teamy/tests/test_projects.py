...
async def test_leader_can_rename_archive_and_delete_project(client):
    ...
    blocked_write = await client.post(
        f"/projects/{project_id}/announcements", json={"title": "Update", "body": "Archived now"}
    )
...
async def test_leader_can_export_project_backup(client):
    ...
    announcement = await client.post(
        f"/projects/{project['id']}/announcements", json={"title": "Export announcement", "body": "Snapshot this."}
    )
