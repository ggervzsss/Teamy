...
async def test_project_members_can_pin_and_unpin_announcements(client):
    ...
    pinned = await client.patch(
        f"/projects/{project['id']}/announcements/{announcement_id}/pin", json={"is_pinned": True}
    )
    assert pinned.status_code == 200
    assert pinned.json()["is_pinned"] is True

    unpinned = await client.patch(
        f"/projects/{project['id']}/announcements/{announcement_id}/pin", json={"is_pinned": False}
    )
...
async def test_past_dated_announcements_are_not_pinned(client):
    ...
    pinned = await client.patch(
        f"/projects/{project['id']}/announcements/{created.json()['id']}/pin", json={"is_pinned": True}
    )
